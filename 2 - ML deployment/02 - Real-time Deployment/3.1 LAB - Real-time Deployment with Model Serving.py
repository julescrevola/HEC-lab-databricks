...
username_extracted = DA.username.split("@")[0]
endpoint_name = f"ML_endpoint_test{username_extracted}"
...
# MAGIC
# MAGIC Let's verify that the endpoint is created and ready to be used for inference. You can either use the endpoint create through the UI (specify the name in the endpoint name) or the endpoint created above through the SDK.

# COMMAND ----------

endpoint_name = <FILL IN>
...
# MAGIC %md
# MAGIC This feature is in Private Preview. You will not have access to it during this lab. Please have a look at the steps in the [documentation](https://docs.databricks.com/en/machine-learning/feature-store/online-tables.html)
...
username_extracted = DA.username.split("@")[0]
fs_endpoint_name = f"ML_endpoint_test{username_extracted}"
try:
  w.serving_endpoints.create_and_wait(
    <FILL IN>
  )
